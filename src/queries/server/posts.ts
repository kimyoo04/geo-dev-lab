...
export async function getPostAPI(id: number | null, params?: { userId?: string; slug?: string }) {
...
export async function getAdjacentPostAPI(
  ...
  if (adjacent?.previous_id) {
    const result = await fetcher<PostAPI>(`/api/v1/post?id=${adjacent?.previous_id}`)
    previousPost = result?.data
  }

  if (adjacent?.next_id) {
    const result = await fetcher<PostAPI>(`/api/v1/post?id=${adjacent?.next_id}`)
