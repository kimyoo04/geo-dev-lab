...
export function useTagAPI(id: number | null, params?: { userId?: string; slug?: string }) {
  const query = setQueryString({ id, ...params })
  const url = query ? `/api/v1/tag?${query}` : null

  const { data, error, isLoading, isValidating, mutate } = useSWR<TagAPI, Error>(url)
...
export function useTagsAPI(
  ...
  const { data, error, isLoading, isValidating, mutate } = useSWR<TagsAPI, Error>(url)
