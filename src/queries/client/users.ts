...
export function useUserAPI(id: string | null = null, params?: { username?: string }) {
  ...
  const { data, error, isLoading, isValidating, mutate } = useSWR<UserAPI, Error>(url)
...
export function useUsersAPI(id: string | null, params?: { page?: number; perPage?: number }) {
  const { session } = useAuth()
  ...
  const { data, error, isLoading, isValidating, mutate } = useSWR<UsersAPI, Error>(url)
